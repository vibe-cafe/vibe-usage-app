...
- Display: `RateLimitCardView` hides disabled providers. Enabled providers still collapse on `.noData` after refresh, but a cold Codex `.noData` remains visible as 「正在读取订阅配额…」 while its network request is active; both showing → side-by-side cards, only one → single full-width card, neither enabled → no quota section.
