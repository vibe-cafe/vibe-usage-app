...
│   │   ├── RateLimitCoordinator.swift # Orchestrates quota refreshes (cache-then-live per provider)
...
│   │   ├── GrokUsageAPI.swift     # Live Grok billing endpoint client (auth.json token, credits GET)
│   │   ├── GrokRateLimitReader.swift # Offline Grok fallback: unified.jsonl billing events
...
└── PopoverView (520px wide, hosted in NSHostingView pinned to panel.contentView)
    ...
    └── dashboardView
        ...
        ├── ScrollView
        │   ├── RateLimitCardView # Codex / Claude / Grok subscription quota cards
...
- Settings exposes separate persisted provider toggles: `codexRateLimitEnabled` (default on, with legacy `rateLimitMonitoringEnabled` fallback), `claudeRateLimitEnabled` (default on after the probe migration), and `grokRateLimitEnabled` (default **off**, so the dashboard stays a two-card Codex/Claude row unless the user opts in). If every toggle is off, no `RateLimitCoordinator` is started and the quota section is hidden entirely.
- Popover open → enabled providers only: `refreshCodexIfNeeded()` / `refreshClaudeIfNeeded()` / `refreshGrokIfNeeded()` (60s debounce each). None of the paths can prompt the user for anything.
...
- **Grok is the same Codex-shaped network-first path against the Grok CLI-proxy billing endpoint.** `GrokUsageAPI` GETs `{cli_chat_proxy}/billing?format=credits` (default `https://cli-chat-proxy.grok.com/v1`; honors `[endpoints] cli_chat_proxy_base_url` in `~/.grok/config.toml` and `$GROK_HOME`) with the OAuth `key` from `~/.grok/auth.json` — a plain-file read, no keychain, no prompts, and the app never runs the refresh grant or rewrites that file. This is the SuperGrok / X Premium+ **shared weekly (or monthly) credits pool**, not the metered developer API (RPS/TPM). API-key-only logins have no session token and are treated as absent. The refresh chain: paint the last live snapshot instantly if the card is empty (persisted to `~/.vibe-usage/grok-rate-limits.json`, SHA-256 scoped to user id + endpoint; expired windows and snapshots over 7 days old are rejected) → live fetch (3 attempts, backoff, 10s timeout; `X-XAI-Token-Auth: xai-grok-cli`) → on 401 re-read auth.json once and retry when the token changed → still-401 falls back to JSONL or surfaces `.unauthorized` → 403 (no consumer pool / team principal) is `.notApplicable` and collapses the card without JSONL → transport errors fall back to `GrokRateLimitReader`. A best-effort 2s `GET /v1/settings` fills `subscription_tier_display` for the plan badge and must not delay an already-fetched percentage. Prefer `https://auth.x.ai::<client-id>` entries over legacy `accounts.x.ai` sessions. Isolated Grok extra roots stay token-scan only; quota is the logged-in account, not per-root. Do **not** spawn `grok agent stdio` (ACP `x.ai/billing` is TUI-wired) and do **not** read grok.com cookies / Chrome Keychain.
- **Claude delegates the live read to a Claude Code binary.** `ClaudeUsageProbe` spawns one (`--safe-mode --no-session-persistence --strict-mcp-config --mcp-config '{"mcpServers":{}}' --tools "" --output-format stream-json --input-format stream-json --verbose`), sends `initialize` then `get_usage` over the stdio control protocol, and parses the reply: `five_hour` / `seven_day` / `seven_day_opus` / `seven_day_sonnet` (each with `utilization` + ISO-8601 `resets_at`), `extra_usage`, and `subscription_type` for the plan badge. ~2.5s, zero tokens (no prompt is ever sent), no keychain prompt — the binary reads its own credentials. Binary discovery goes `VIBE_USAGE_CLAUDE_BIN` → user-installed CLI (`~/.local/bin/claude` and friends) → the newest copy Claude Desktop manages (`~/Library/Application Support/Claude/claude-code/<version>/claude.app/Contents/MacOS/claude`; older retained Desktop versions are not retried). The Desktop bundle is deliberately **last**: it exists to cover machines with no CLI at all and must not take over on machines that have one. Candidates share one 25s overall budget and each gets at most 8s, so an executable that hangs cannot multiply the loading time. Stdout uses cancellable async line iteration; panel close/toggle-off also terminates the active `Process`, so neither a blocked read nor a descendant retaining the pipe can hold the refresh open. `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC` **must** be stripped from the child environment — it suppresses the usage fetch and makes the probe answer `rate_limits: null`. `CLAUDE_CONFIG_DIR` is preserved. `rate_limits_available: false` (API key / Bedrock / Vertex) is a permanent answer and collapses the card; a null `rate_limits` on a subscription account is retryable and falls back to cache.
...
- **Freshness surfaces in the UI, not just logs.** `ProviderRateLimit.dataAsOf` records when the numbers were produced (live fetch ≈ now; Codex JSONL event timestamp; Grok unified.jsonl `ts`; Claude's own `fetchedAtMs` / Desktop's sample time). A minute-driven `TimelineView` makes the card footer show and continuously update 「数据截至 N 分钟前」 once age exceeds 5 minutes, plus 「重置券 ×N」 when Codex reports available reset credits. `isCodexRateLimitRefreshing` / `isClaudeRateLimitRefreshing` / `isGrokRateLimitRefreshing` on AppState drive a mini spinner in the card header while a refresh is in flight (Codex/Grok ~1s of network latency, Claude ~2.5s of subprocess round trip).
...
- Display: `RateLimitCardView` hides providers the user turned off. A cold `.noData` stays visible as 「正在读取订阅配额…」 while that provider's refresh is in flight (every enabled leg now has real latency); two showing → side-by-side cards, one → single full-width card, three → first two in a 2-col grid and the third full-width below (520px popover never squeezes three columns), none enabled → no quota section. Grok does not reserve a 5h slot; its shared pool is labeled `7d` or `30d` from window duration.
- A settled `.noData` means the provider is absent or plan limits do not apply. If another enabled provider has data, the empty provider stays visible with a quiet no-quota message so the Settings toggle remains truthful; only an all-`.noData` row collapses into the capability notice. A concrete endpoint/probe failure with no cache becomes semantic `.retryableError` (localized in the view) and keeps the provider card plus retry button visible; that retry refreshes only its own provider (Claude retry does not hit Codex or Grok, and vice versa). Provider transport errors conform to `RateLimitFetchError`, whose shared classification prevents the coordinator from depending on provider-specific error enums. `.disabled` is unreachable now and only survives as a landing for snapshots persisted by pre-0.5.7 builds.
- Terminology: code stays on `RateLimit` (matches the `rate_limits` field Codex/Claude return; Grok's live field is `creditUsagePercent` mapped onto the same card model); user-facing copy uses 「订阅配额」. Settings toggles should use display-oriented copy: 「显示 Codex 订阅配额」 / 「显示 Claude 订阅配额」 / 「显示 Grok 订阅配额」. When `ClaudeUsageProbe.primarySourceKind() == .desktop` (Desktop installed, no CLI) the Claude toggle gets a 「数据来源：Claude Desktop」 subtitle — the one case where the data comes from something the user did not install themselves. With a CLI present there is nothing to explain and the note stays hidden; it resolves once at launch (`AppState.claudeUsesDesktopBundledCLI`) since it only depends on what is on disk.
- `RateLimit.swift`'s `sevenDayOpus`/`sevenDaySonnet`/`extraUsage` are populated by the Claude probe (Max plans expose the per-model weekly windows); Grok also fills `extraUsage` from on-demand cap/used when the cap is > 0. The card does not render those extra rows yet. `.unauthorized` is real again: `CodexUsageAPI` and `GrokUsageAPI` map a post-reload 401 to it, and the card renders a re-login hint with a retry button.
- **Codex / Grok JSONL staleness policy (stricter than Claude's).** Codex's `rate_limits.primary/secondary.resets_at` is anchored to a true rolling window, so a `resets_at` already in the past proves that window has rolled over and the snapshot's `used_percent` is from the *previous* window. `CodexRateLimitReader.parseWindow` drops any such slot (per-window — 5h and 7d expire independently), and `read()` returns `.noData` if both slots are expired so the card collapses. Grok's `currentPeriod.end` is the same kind of rollover proof: `GrokRateLimitReader` drops an expired latest billing event rather than walking back to an older week. Without this filter the reader would display a stale percentage indefinitely (e.g. an 8% reading hanging around 12 days after that window's `resets_at`). The Claude reader is intentionally more lenient — it keeps `used_percentage` and only suppresses the elapsed-time bar when stale — because Claude's payload has no equivalent "this window has provably rolled" signal; staleness there just means Claude Code is idle (and the 「数据截至」 footer note communicates the age).
