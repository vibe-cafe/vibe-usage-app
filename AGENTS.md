...
- **Codex is network-first with a local fallback.** `CodexUsageAPI` GETs the zero-quota usage endpoint (`{base}/wham/usage`, base honors `chatgpt_base_url` in `~/.codex/config.toml` and `$CODEX_HOME`) with the OAuth access token + account id from `~/.codex/auth.json` — a plain-file read, no keychain, no prompts. This keeps the card fresh while Codex is idle and adds facts the JSONL can't provide: live `plan_type`, "window not enforced" semantics (see below), and `rate_limit_reset_credits`. The refresh chain: paint the cached JSONL snapshot instantly if the card is empty → live fetch (3 attempts, backoff, 10s timeout) → on 401 re-read auth.json once and retry (the CLI rotates tokens; we deliberately never run the OAuth refresh grant or write auth.json) → still-401 falls back to JSONL or surfaces `.unauthorized` → transport errors fall back to `CodexRateLimitReader`'s session-JSONL scan, keeping a previous good snapshot rather than collapsing the card on a network blip.
