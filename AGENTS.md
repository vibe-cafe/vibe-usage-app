...
- Popover open → enabled providers only: `refreshCodexIfNeeded()` and/or `refreshClaudeIfNeeded()` (60s debounce each). Both are auth-free local-file reads — no prompts.
- Footer "更新数据" / card retry buttons → `refreshAll()` (the coordinator still skips disabled providers).
